...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", limit=5, start=0):
        """
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&start=" + str(start) + "&count=" + str(limit) + "&lat=" +
                          str(latitude) + "&lon=" + str(longitude) + "&sort=rating&order=desc&cuisines=" + str(cuisines), headers=headers).content).decode("utf-8")
    ...
    def getTopRestaurants(self, loc, cuisine, price, count=10):
        location_detail = self.get_location(loc, 1)
    ...
    def getRestaurant(self, cuisine, lat, lon, start, price, count):
        results = self.restaurant_search(
            "", lat, lon, cuisine, limit=20, start=start)
        ...
        if d['results_found'] == 0:
            ...
            for restaurant in d['restaurants']:
                ...
                if((price_min <= average_cost) and (average_cost <= price_max)):
                    restaurantDetails = {}
                    restaurantDetails["name"] = restaurant['restaurant']['name']
                    restaurantDetails["address"] = restaurant['restaurant']['location']['address']
                    restaurantDetails["rating"] = restaurant['restaurant']['user_rating']["aggregate_rating"]
                    restaurantDetails["average_cost_for_two"] = average_cost
                    results.append(restaurantDetails)
                    current_count = current_count+1

        return (results, d['results_found'] > 0)
